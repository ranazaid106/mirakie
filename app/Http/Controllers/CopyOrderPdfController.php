...
class CopyOrderPdfController extends Controller
{
...
public function generateInvoice(Request $request ,$id)
{
    ...
    $pdf = PDF::loadView('Admin.generate.copy_order_pdfGenerate', $data);
 ...
 public function multiplesgenerate(Request $request)
 {
    ...
    DB::raw('(select name from users where copy_short_orders.user_id  = users.id ) as user_name'),
    // DB::raw('(select created_at from copy_short_orders  where copy_short_orders.short_product  = shortorders.id ) as user_create_date'),
    )
    ->leftJoin("users","copy_short_orders.user_id","users.id")
    ->leftJoin("products","copy_short_orders.short_product","products.id")
    ...
    $data = [ 'orders' => $orders ]; 
    $pdf = PDF::loadView('Admin.generate.copy_order_pdf_multiples_generate', $data );
