...
<div class="row">
    <div class="col-md-12">
        <h3 class="title-5 m-b-35">order Update</h3>
        ...
        <div class="table-responsive table-responsive-data2" style="overflow-x: auto;">
            <table class="table table-data2 load_datatable orderTable" id="orderTable" style=" margin-bottom: 20px;">
                ...
                <tbody class="append_order_datas">
                    ...
                    <form id="forms{{ $item->short_orders_id }}_data"  action="{{ route('updateForm', $item->short_orders_id) }}" method="POST">
                            
                                @csrf
                <td>
                    <input type="hidden" name="order_id"  value="{{$item->short_orders_id}}" >

                    <select name="comfirmation" class="form-select" aria-label="Default select example" style="width: 195px">
                        <option value="">Select Confirmation</option>
                        <option value="confirmed" {{ $item->comfirmation == 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                        <option value="No Response" {{ $item->comfirmation == 'No Response' ? 'selected' : '' }}>No Response</option>
                        <option value="cancel" {{ $item->comfirmation == 'cancel' ? 'selected' : '' }}>cancel</option>
                        <option value="other reason" {{ $item->comfirmation == 'other reason' ? 'selected' : '' }}>Other Reason</option>
                    </select>
                    
                </td>
                <td>
                    <select name="status" class="form-select" aria-label="Default select example" style="width: 140px">
                        <option value="">Select Status</option>
                        <option value="Delivered" {{ $item->status == 'Delivered' ? 'selected' : '' }}>Delivered</option>
                        <option value="cancel" {{ $item->status == 'cancel' ? 'selected' : '' }}>Cancel</option>
                        <option value="other reason" {{ $item->status == 'other reason' ? 'selected' : '' }}>Other Reason</option>
                    </select>
                       
                </td>
                <td>
                   
                     @if($item->commision == "null")
                     <div class="table-data-feature" style=";width:228px" > 
                       
                       
                        <input  style="width: 228px;"  type="text" name="commision" class="form-control" placeholder="Enter the Commission">
                       </div>
                    @else
                   <div class="table-data-feature" style=";width:228px" > 
                   
                       
                    <input  style="width: 228px;" value="{{ $item->commision }}"  type="text" name="commision" class="form-control" placeholder="Enter the Commission">
                       </div>
                    @endif
                ...
                <td>
                     ...
                     @if($item->commision == "null")
                    <div class="table-data-feature" style=";width:228px"> 
                        <textarea  rows="1"  style="width: 228px;" type="text"  name="reason" class="form-control" placeholder="Enter the Reason"></textarea>
                        {{-- <input style="width: 228px;" type="text"  name="reason" class="form-control" placeholder="Enter the Reason"> --}}
                       </div>

                    @else
                  <div class="table-data-feature" style=";width:228px"> 
                    <textarea  rows="1" style="width: 228px;" type="text"  name="reason" class="form-control" placeholder="Enter the Reason">{{ $item->reason }}</textarea>

                        {{-- <input style="width: 228px;" type="text" value="{{ $item->reason }}" name="reason" class="form-control" placeholder="Enter the Reason">
                       </div> --}}
        ...
        <div class="modal-content">
            ...
            <div class="modal-body" style="height: 100%">
                <div class="container" style="height: 800px;">
                    ...
                    $delivery = App\Models\CopyShortOrder::distinct()->pluck('status');
                    ...
                    <div class="row mb-1">
                        <div class="col-md-12">
                            ...
                            <select id="delivery_status" name="delivery_status" class="form-select" aria-label="Default select example" >
                                <option value="">Select status</option>
                    ...
                    <div class="row mb-1">
                        <div class="col-md-12">
                            <label><b>confirmation</b>  (Optional) </label>

                            <select name="confirmation" id="confirmation" class="form-select" aria-label="Default select example" >
                                <option value="">Select Confirmation</option>
...
<script>
  ...
  if (allVals.length <= 0) {
            ...
            $.ajax({
                url: '/update_order_generate_excels',
                data: params,
                type: 'GET',
                xhrFields: {
...
<script>
    ...
    $(document).on('click', '#search_filter_order_customer', function(e) {
      ...
      var delivery_status1  = $('#delivery_status').val();
      var confirmation1  = $('#confirmation').val();
      var country_name1  = $('#country_name').val();
      var agen_name1  = $('#agen_name').val();
    ...
    var v_token = "{{csrf_token()}}";
       var params1 = {country_name:country_name1, _token: v_token , agen_name: agen_name1 , zip_code: zip_code1 , product_name: product_name1 ,  date_from: date_from1 , date_to:date_to1 ,delivery_status:delivery_status1 , confirmation:confirmation1 };

    $.ajax({
        type: 'post',
        data: params1,
        url: "/copy_advance_searchings_customer",
                ...
                var tableRow = `
                    <tr class="tr-shadow select_${$item.id}" style="text-align: center">
                        ...
                        ${$item.created_at ?
                            ...
                            `<td class="desc"><span> not found</span></td>`}
                        <td class="desc">${$item.short_delivery_date ?? 'empty'}</td>
                        <form id="form_${$item.short_orders_id}_data" action="'orderupdateForm'/${($item.short_orders_id)}" method="POST">
                            @csrf
                            <td>
                                <input type="hidden" name="order_id" id="order_id"  value="${$item.short_orders_id}" >
                                <select name="comfirmation" id="comfirmation"  class="form-select" aria-label="Default select example" style="width: 195px">
                                    <option value="">Select Confirmation</option>
                                    <option value="confirmed" ${$item.comfirmation == 'confirmed' ? 'selected' : ''}>Confirmed</option>
                                    <option value="No Response" ${$item.comfirmation == 'No Response' ? 'selected' : ''}>No Response</option>
                                    <option value="Cause" ${$item.comfirmation == 'Cause' ? 'selected' : ''}>Cause</option>
                                </select>
                            </td>
                            <td>
                                <select name="status"  id="status"  class="form-select" aria-label="Default select example" style="width: 140px">
                                    <option value="">Select Status</option>
                                    <option value="Delivered" ${$item.status == 'Delivered' ? 'selected' : ''}>Delivered</option>
                                    <option value="cancel" ${$item.status == 'cancel' ? 'selected' : ''}>cancel</option>
                                </select>
                            ...
                            <td>
                                <div class="table-data-feature" style="padding-left:30px;width:215px" > 
                                    <input type="text" id="commision"  value="${$item.commision}" name="commision" class="form-control" placeholder="Enter the Commission">
                                </div>
                            </td>
                            <td>
                                <div class="table-data-feature" style="padding-left:30px;width:185px"> 
                                    <input type="text" id="reason" value="${$item.reason}" name="reason" class="form-control" placeholder="Enter the Reason">
                                </div>
                            </td>
...
<script>
    ...
    var v_token = "{{csrf_token()}}";
       var params = {country_name:country_name, _token: v_token , zip_code: zip_code , product_name: product_name ,  date_from: date_from , date_to:date_to ,delivery_status:delivery_status , confirmation:confirmation ,agen_name:agen_name };

    $.ajax({
        type: 'post',
        data: params,
        url: "/copy_advance_searchings",
        success: function(data) {
            if(data.status == 202){
                $('.correct_agen_plz').html("");
                $('.filter_loarder_btn').hide();
                ...
                var tableRow = `
                    <tr class="tr-shadow select_${$item.id}" style="text-align: center">
                        ...
                        <td class="desc">${$item.short_delivery_date ?? 'empty'}</td>
                            ...
                            <form id="form${$item.short_orders_id}_data" action="'orderupdateForm'/${($item.short_orders_id)}) }}" method="POST">
                              @csrf 


                            <td>
                                <input type="hidden" name="order1_id_${$item.short_orders_id}"  value="${$item.short_orders_id}" >
                                <select name="comfirmation" id="comfirmation_${$item.short_orders_id}" class="form-select" aria-label="Default select example" style="width: 195px">
                                    <option value="">Select Confirmation</option>
                                    <option value="confirmed" ${$item.comfirmation == 'confirmed' ? 'selected' : ''}>Confirmed</option>
                                    <option value="No Response" ${$item.comfirmation == 'No Response' ? 'selected' : ''}>No Response</option>
                                    <option value="cancel" ${$item.comfirmation == 'cancel' ? 'selected' : ''}>cancel</option>
                            ...
                            <td>
                                <select name="status" id="status_${$item.short_orders_id}" class="form-select" aria-label="Default select example" style="width: 140px">
                                    <option value="">Select Status</option>
                                    <option value="Delivered" ${$item.status == 'Delivered' ? 'selected' : ''}>Delivered</option>
                                    <option value="Cancel" ${$item.status == 'Cancel' ? 'selected' : ''}>Cancel</option>
                                </select>
                            ...
                            <td>
                                <div class="table-data-feature" style="padding-left:30px;width:215px" > 
                                    <input id="commision_${$item.short_orders_id}" type="text" value="${$item.commision}" name="commision" class="form-control" placeholder="Enter the Commission">
                                </div>
                            </td>
                            <td>
                                <div class="table-data-feature" style="padding-left:30px;width:185px"> 
                                    <input id="reason_${$item.short_orders_id}" type="text" value="${$item.reason}" name="reason" class="form-control" placeholder="Enter the Reason">
                                </div>
                            </td>
                            <td>
                                <div class="table-data-feature" style="padding-left:30px"> 

                                    <button type="button" class="form-control btn btn-primary " onclick="submitForms('${$item.short_orders_id}')">Save</button>
                                </form>
                             

                               
                                    <a  data-id="${$item.id}" style="cursor: pointer;color:white" class="btn btn-danger delete_order_one_item ml-2 " >Delete</a> 
                                </div>
                            </td> 
                            <td>
                            <a  href="{{ url('copy_order/${$item.id}/customer/${$item.id}/pdf' ) }}"   data-id="${$item.id}" style="right:45%;cursor: pointer;" class="item " data-toggle="tooltip" data-placement="top" title="GENERATE PDF"><span class="material-symbols-outlined"> picture_as_pdf</span></a>
...
<script>
    ...
    $(document).on('click', '.form_submit1111', function(e) {
                        e.preventDefault();
            var dataId = $(this).attr("data-id");
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        $.ajax({
                                            type: 'get',
                                            data: params,
                                            url: "/copy_multiplesgenerateInvoice", 
                                            xhrFields: {
                                                        responseType: 'blob'
                                                    },
...
// alert(formId);
        ...
        var comfirmation = $("#comfirmation_"+ formId).val();
        var commision = $("#commision_"+ formId).val();
        var reason = $("#reason_"+ formId).val();
        var status = $("#status_"+ formId).val();
        var order1_id = $("#order1_id_"+ formId).val();
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route('orderupdateForm', '') }}/' + formId,
             
                data: { comfirmation: comfirmation, commision: commision ,reason: reason,status: status,order1_id: order1_id,_token: v_token },
                success: function(response) {
                    // Handle the success response
                    Swal.fire(
                                    'Order Updated!',
